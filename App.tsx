...
import { GameState, PRIZE_LADDER, SAFE_HAVENS, User, GameSettings, Question } from './types';
...
function App() {
    ...
    if (appMode === 'game' && gameState.gameStatus === 'playing' && !gameState.isLocked && !showAudienceModal && !showPhoneModal && !isTransitioning) {
      interval = setInterval(() => {
        setTimer((prev) => {
          ...
          if (prev <= 10) playSound(SOUNDS.TICK);
          return prev - 1;
        });
      }, 1000);
    }
  ...
  useEffect(() => {
    if (gameState.gameStatus === 'won') playSound(SOUNDS.WIN);
    else if (gameState.gameStatus === 'lost') playSound(SOUNDS.LOSE);
  ...
  const handleOptionClick = (index: number) => {
    ...
    if (gameState.doubleDipActive) {
      if (index === currentQ.correctAnswerIndex) {
        ...
        setTimeout(() => revealAnswer(index, true), 1500);
      } else {
    ...
    setTimeout(() => revealAnswer(index, index === currentQ.correctAnswerIndex), 2000);
  };

  const revealAnswer = (index: number, isCorrect: boolean) => {
    setGameState(prev => ({ ...prev, isCorrect }));
    isCorrect ? playSound(SOUNDS.CORRECT) : playSound(SOUNDS.WRONG);
    ...
    setTimeout(() => {
      if (isCorrect) {
        ...
        SAFE_HAVENS.forEach(idx => {
            if (gameState.currentQuestionIndex > idx) safeMoney = PRIZE_LADDER[idx];
        });
  ...
  if (appMode === 'menu' && user) {
    return (
      <MainMenu 
        ...
        onLogout={() => { setUser(null); setAppMode('auth'); }} 
        settings={settings}
  ...
  if ((gameState.gameStatus === 'won' || gameState.gameStatus === 'lost') && questions.length > 0) {
      return (
          <div className="min-h-screen flex flex-col items-center justify-center bg-slate-900 text-white p-6 relative animate-zoomIn">
              <div className="text-center z-10">
                  ...
                  <div className="flex gap-4 justify-center">
                    <button onClick={startGame} className="px-8 py-3 bg-amber-600 hover:bg-amber-700 rounded-full font-bold text-lg transition-all shadow-lg">Play Again</button>
                    <button onClick={() => setAppMode('menu')} className="px-8 py-3 bg-slate-700 hover:bg-slate-600 rounded-full font-bold text-lg transition-all shadow-lg">Main Menu</button>
                  </div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col md:flex-row relative overflow-hidden">
      ...
      <div className="md:hidden fixed top-4 right-4 z-40">
        <button onClick={() => setShowPrizeLadder(!showPrizeLadder)} className="bg-amber-600 p-2 rounded-full shadow-lg text-white">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className={`flex-1 flex flex-col relative z-10 h-screen transition-opacity duration-500 ${isTransitioning ? 'opacity-0' : 'opacity-100'}`}>
        <div className="p-4 flex justify-between items-center">
             ...
             <div className="text-right">
                 <p className="text-xs text-slate-400">Prize at Stake</p>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8">
            <div className="w-full max-w-4xl text-center mb-8 animate-slideInUp delay-100">
                <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border-y-2 border-amber-600/50 py-8 px-6 relative shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                    {currentQ.questionHindi && <h3 className="text-lg md:text-xl text-slate-400 mb-2 font-serif">{currentQ.questionHindi}</h3>}
                    <h2 className="text-xl md:text-3xl font-bold text-white leading-relaxed">{currentQ.question}</h2>
            ...
            <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 px-2">
                {currentQ.options.map((option, index) => (
                    <button key={index} onClick={() => handleOptionClick(index)} disabled={gameState.isLocked && !gameState.doubleDipActive} className={`relative w-full py-4 px-8 rounded-full border-2 transition-all duration-200 text-left flex items-center group animate-slideInUp ${getButtonColor(index)}`} style={{ animationDelay: `${200 + (index * 100)}ms` }}>
                        <span className="text-amber-500 font-bold mr-4 text-xl group-hover:text-white transition-colors">{String.fromCharCode(65 + index)}:</span>
        ...
        <div className="p-6 bg-slate-950 border-t border-slate-800 flex flex-col items-center">
            <div className="flex justify-center gap-4 md:gap-8 mb-4">
                <LifelineButton type="50:50" label="50:50" icon={<Icon5050 />} isUsed={gameState.lifelines.fiftyFifty} isActive={false} onUse={() => !gameState.isLocked && use5050()} />
                <LifelineButton type="audience" label="Audience Poll" icon={<IconAudience />} isUsed={gameState.lifelines.audiencePoll} isActive={showAudienceModal} onUse={() => !gameState.isLocked && useAudience()} />
                <LifelineButton type="phone" label="Phone A Friend" icon={<IconPhone />} isUsed={gameState.lifelines.phoneAFriend} isActive={showPhoneModal} onUse={() => !gameState.isLocked && usePhone()} />
                <LifelineButton type="double" label="Double Dip" icon={<IconDouble />} isUsed={gameState.lifelines.doubleDip} isActive={gameState.doubleDipActive} onUse={() => !gameState.isLocked && useDoubleDip()} />
            </div>
            <AdBanner className="mt-2" />
        </div>
      </div>
      <PrizeLadder currentQuestionIndex={gameState.currentQuestionIndex} isOpen={showPrizeLadder} close={() => setShowPrizeLadder(false)} />
      {showAudienceModal && <div onClick={() => setShowAudienceModal(false)} className="animate-zoomIn"><AudienceGraph correctIndex={currentQ.correctAnswerIndex} /></div>}
      {showPhoneModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-zoomIn">
              <div className="bg-slate-800 border-2 border-amber-500 rounded-xl max-w-md w-full p-6 shadow-2xl relative">
                  <div className="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-amber-500 rounded-full p-4 border-4 border-slate-900"><IconPhone /></div>
                  ...
                  <div className="bg-slate-700 p-4 rounded-lg text-slate-200 italic mb-6 relative">
                      "Hello {user?.name}! Based on my knowledge, the answer is likely <strong className="text-white not-italic mx-1">{currentQ.options[currentQ.correctAnswerIndex]}</strong>."
                  </div>
                  <button onClick={() => setShowPhoneModal(false)} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded uppercase">End Call</button>
              </div>
          </div>
      )}
    </div>
